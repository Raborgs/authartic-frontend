...
export default function PaginatedTable() {
  ...
  const {
    data: allFontsData,
    isLoading: isAllFontsLoading,
    error: allFontsError,
    refetch: allFontDataRefetch,
  } = useGetAdminAllFontsQuery({
    page,
    limit: pageSize,
  ...
  const [
    createFont,
    { isLoading: isCreateFontLoading, error: createFontError },
  ] = useCreateAdminNewFontMutation();
  const [
    updateFont,
    { isLoading: isUpdateFontLoading, error: updateFontError },
  ] = useUpdateAdminFontMutation();
  const [
    deleteFont,
    { isLoading: isDeleteFontLoading, error: deleteFontError },
  ] = useDeleteAdminFontMutation();
    ...
    try {
      if (selectedFont) {
        ...
        setRows((prevRows) =>
          [
            ...prevRows.map((row) =>
              row.id === updatedFont.id ? updatedFont : row
            ),
          ].sort((a, b) => a.id - b.id)
        );
      ...
      toast.error(
        error?.message ||
          error?.data?.message ||
          "An error occurred while saving the font. Please try again."
      );
    }
  };
            ...
            <div>
              ...
              >
                {options.map((option, index) => (
                  <MenuItem
                    key={index}
                    selected={option.isActive === isActiveFont}
          ...
          <Box sx={{ height: "450px", overflow: "auto" }}>
            ...
            {allFontsError && (
              <Typography variant="h5" color={"error"}>
                {allFontsError?.data?.message || "Error loading fonts"}
              </Typography>
            )}
      ...
      <Dialog open={openEditModal} onClose={handleModalClose}>
        ...
        <DialogContent>
          ...
          <TextField
            ...
            onChange={(e) =>
              setFontData({ ...fontData, family: e.target.value })
            }
          />
        </DialogContent>
        <DialogActions>
